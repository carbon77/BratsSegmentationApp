<template>
  <Panel :header="t('uploadPanelTitle')">
    <Message v-if="errorMessage" severity="error">{{ errorMessage }}</Message>
    <div class="flex flex-col gap-3">
      <Button :label="t('createScan')" icon="pi pi-cloud-upload" :loading="isUploading"
        :disabled="!allModalitiesSelected || isUploading" @click="$emit('submit')" />
      <FileUpload class="w-full" mode="basic" name="demo[]" customUpload multiple accept=".nii,.nii.gz"
        :chooseLabel="t('chooseFourFiles')" chooseIcon="pi pi-folder-open" @select="onAllFilesSelected" />
    </div>

    <Divider />

    <Panel class="p-0" :header="t('requiredModalities')">
      <DataTable :value="requiredRows" size="small" responsiveLayout="scroll">
        <Column field="label" :header="t('title')" />
        <Column :header="t('status')">
          <template #body="slotProps">
            <Tag :value="modelValue[slotProps.data.key] ? t('selected') : t('missing')
        ...
        <Column :header="t('actions')">
          <template #body="slotProps">
            <FileUpload mode="basic" customUpload accept=".nii,.nii.gz" :chooseLabel="modelValue[slotProps.data.key] ? t('replace') : t('choose')
              " chooseIcon="pi pi-upload" @select="(event) => onFileSelected(event, slotProps.data.key)" />
          </template>
        </Column>
      </DataTable>
    </Panel>
    ...
    <Panel :header="t('optionalTrueMask')">
      <DataTable :value="optionalRows" size="small" responsiveLayout="scroll">
        ...
        <Column :header="t('status')">
          <template #body="slotProps">
            <Tag :value="modelValue[slotProps.data.key] ? t('selected') : t('optional')
...
const requiredRows = computed(() =>
  modalities.map((key) => ({ key, label: key.toUpperCase() })),
);
const optionalRows = [{ key: "true_mask", label: "TRUE_MASK" }];
const allModalitiesSelected = computed(() =>
  modalities.every((m) => Boolean(props.modelValue[m])),
);

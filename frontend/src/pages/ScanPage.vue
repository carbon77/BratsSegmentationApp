<template>
  <section>
    <Card>
      <template #title>
        <span>{{ title || t("scanFallback", { caseId }) }}</span>
      </template>
      <template #content>
        <InputGroup>
          <InputText v-model="editedTitle" :placeholder="t('scanTitlePlaceholder')" />
          <Button :label="t('saveTitle')" icon="pi pi-check" :disabled="!editedTitle.trim() || editedTitle === title"
            :loading="isSavingTitle" @click="saveTitle" />
          <Button :label="t('deleteScan')" icon="pi pi-trash" severity="danger" outlined :loading="isDeleting"
            @click="removeScan" />
        </InputGroup>
      </template>
    </Card>
    ...
    <ProgressSpinner v-if="isLoading" class="size-8" strokeWidth="6" />

    <template v-else>
      <Message v-if="errorMessage" severity="error">{{ errorMessage }}</Message>
      <Message v-if="metadataSavedMessage" severity="success">{{
        metadataSavedMessage
      }}</Message>
      <Message v-if="orthancMessage" severity="success">{{
        orthancMessage
      }}</Message>

      <div class="flex gap-5 item-start flex-wrap min-h-[42rem]">
          ...
          <Panel v-else :header="t('dicomMetadata')">
            ...
            <DataTable :value="metadataFields" size="small" responsiveLayout="scroll">
              ...
              <Column :header="t('value')">
                <template #body="slotProps">
                  <Dropdown v-if="slotProps.data.type === 'select'" v-model="editedDicomMetadata[slotProps.data.key]"
                    :options="slotProps.data.options" optionLabel="label" optionValue="value"
                    :placeholder="t('dicomMetadataEmpty')" showClear />
                  <InputText v-else v-model="editedDicomMetadata[slotProps.data.key]"
                    :type="slotProps.data.type || 'text'" :placeholder="t('dicomMetadataEmpty')" />
                </template>
              </Column>
            </DataTable>
          </Panel>
        ...
        <div class="min-w-64 flex-[1_1_20rem]">
          <template v-if="activeSection === 'slices'">
            <Panel :header="t('dicomExportSettings')">
              <div class="flex justify-center flex-col gap-3">
                <Dropdown v-model="dicomModality" :options="modalityOptions" optionLabel="label" optionValue="value"
                  :placeholder="t('chooseModality')" />
                <ButtonGroup>
                  <Button label="DICOM" icon="pi pi-file-export" outlined :loading="isConvertingDicom"
                    :disabled="!dicomModality || isSendingOrthanc" @click="downloadDicom" />
                  <Button label="Orthanc" icon="pi pi-send" outlined :loading="isSendingOrthanc"
                    :disabled="!dicomModality || isConvertingDicom" @click="sendToOrthanc" />
                </ButtonGroup>
              </div>
            </Panel>
          </template>

          <template v-else-if="activeSection === 'metrics'">
            <Panel :header="t('metricsExportSettings')">
              <ButtonGroup>
                <Button label="JSON" icon="pi pi-download" outlined @click="downloadMetricsJson" />
                <Button label="CSV" icon="pi pi-table" outlined @click="downloadMetricsCsv" />
              </ButtonGroup>
            </Panel>
          </template>

          <template v-else>
            <Panel :header="t('metadataSettings')">
              <Button :label="t('saveDicomMetadata')" icon="pi pi-save" :loading="isSavingMetadata"
...
const metadataFields = [
  ...
  { key: "patient_id", labelKey: "dicomPatientId" },
  {
    ...
    labelKey: "dicomPatientBirthDate",
    type: "date",
  },
  {
    key: "patient_sex",
    labelKey: "dicomPatientSex",
    type: "select",
    options: sexOptions,
  },
...
function dicomMetadataPayload() {
  return Object.fromEntries(
    Object.entries(editedDicomMetadata).map(([key, value]) => [
      key,
...
function downloadMetricsJson() {
  ...
  const payload = JSON.stringify(
    { case_id: props.caseId, metrics: metrics.value },
    null,
    2,
  );
...
function downloadMetricsCsv() {
  ...
  const csv = [
    "metric,value",
    ...rows.map(([metric, value]) => `${metric},${JSON.stringify(value)}`),
  ].join("\n");
...
async function sendToOrthanc() {
  ...
  try {
    ...
    orthancMessage.value = t("dicomOrthancSent", {
      count: result.instances_uploaded,
    });
  } catch {
...
async function loadScanDetails() {
  try {
    ...
    title.value =
      currentScan?.title || t("scanFallback", { caseId: props.caseId });

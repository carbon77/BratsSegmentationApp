<template>
  <div>
    <Toolbar :style="{ minHeight: '3rem', paddingBlock: '0.25rem' }">
      ...
      <template #end>
        <Button icon="pi pi-language" text rounded size="small" :aria-label="t('language')" aria-haspopup="true"
          aria-controls="language_menu" @click="toggleLanguageMenu" />
        <Menu id="language_menu" ref="languageMenu" :model="languageMenuItems" popup />

        <Button icon="pi pi-palette" text rounded size="small" :aria-label="t('theme')" aria-haspopup="true"
          aria-controls="theme_menu" @click="toggleThemeMenu" />
    ...
    <Dialog v-model:visible="showLogoutDialog" modal :header="t('logoutConfirmTitle')" :style="{ width: '28rem' }">
